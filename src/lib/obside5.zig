...
test "obside5 adding bids uniformly" {
    std.debug.print("Start OBSide5 Adding Bids Uniformly\n", .{});
    const seed = 100;
    ...
    std.debug.print("End OBSide5 Adding Bids Uniformly\n\n", .{});
}

test "obside5 adding bids normally around best price" {
    std.debug.print("Start OBSide5 Adding Bids Normally\n", .{});
    const seed = 100;
    const rand = std.rand.DefaultPrng.init(seed);
    const rng = @constCast(&rand).random();
    ...
    std.debug.print("End OBSide5 Adding Bids Normally\n\n", .{});
}
