...
pub fn OrderBookSide(comptime side: comm.Side, comptime size: usize) type {
    return struct {
        ...
        pub fn addOrder(self: *Self, order: comm.Order) void {
            var pos: usize = 0;
            while (pos < self.orders.len) {
                if (comptime side == .buy) {
                    if (order.price >= self.orders.buffer[pos].price) break;
                } else {
                    if (order.price <= self.orders.buffer[pos].price) break;
                }
                pos += 1;
            ...
            if (pos == self.orders.len) {
                self.orders.append(.{ .price = order.price, .qty = order.qty }) catch {
                    // we can ignore this error since nothing gets
                    // added in the case of an overflow.
                };
            } else if (self.orders.buffer[pos].price == order.price) {
                self.orders.buffer[pos].qty = order.qty;
                return;
            } else {
...
test "obside2 adding bids uniformly" {
    std.debug.print("Start OBSide2 Adding Bids Uniformly\n", .{});
    const seed = 100;
    ...
    std.debug.print("End OBSide2 Adding Bids Uniformly\n\n", .{});
}

test "obside2 adding bids normally around best price" {
    std.debug.print("Start OBSide2 Adding Bids Normally\n", .{});
    const seed = 100;
    const rand = std.rand.DefaultPrng.init(seed);
    const rng = @constCast(&rand).random();
    ...
    std.debug.print("End OBSide2 Adding Bids Normally\n\n", .{});
}
